...
export const human = (value: string): string => value
  .replaceAll(/([a-z])([A-Z])/gu, "$1 $2")
  .replaceAll(/[._-]+/gu, " ")
  .replaceAll(/\s+/gu, " ")
  .trim();

export const task = (unit: InterpretationUnit): string => {
...
export const correctionInstruction = (unit: InterpretationUnit): string => {
  const lines = [
...
export const lexicon = (unit: InterpretationUnit): string[] => {
...
export const route = (unit: InterpretationUnit): Route => {
...
export const acceptedNarratives = (earlier: Readonly<Record<string, unknown>>): NarrativeEntry[] =>
