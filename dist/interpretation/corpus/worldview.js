...
const shieldTechnicalProperNames = (value) => technicalProperNames.reduce((current, name) => current.replaceAll(new RegExp(`\\b${name.replaceAll(" ", "\\s+")}\\b`, "giu"), " technical-point "), value);
const rules = [
    {
...
export const auditWorldviewText = (value) => {
...
const finalChartNonInterpretivePath = (path) => path === "astral-chart.subject"
    || path.startsWith("astral-chart.subject.")
    || path === "astral-chart.provenance"
    || path.startsWith("astral-chart.provenance.");
export const auditWorldviewObject = (value, path = "$") => {
    const findings = [];
    const visit = (current, currentPath, key) => {
        ...
        if (key === "sourceRefs" || key === "forbiddenClaims" || key === "composition" || finalChartNonInterpretivePath(currentPath))
            return;
...
const categoryPresent = (findings, category) => findings.some((finding) => finding.category === category);
export const auditSourceNeutrality = (passage) => {
...
export const worldviewFailureMessages = (audit) => audit.findings.map((finding) => {
...
export const assertAgnosticText = (value, context) => {
