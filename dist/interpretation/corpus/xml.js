export const corpusXmlFormat = "astral-corpus-xml/1.0.0";
const sourceRoles = ["calculation", "semantic", "architecture"];
const reviewStatuses = ["approved", "pending", "rejected"];
const atomKinds = ["entity", "domain", "style", "relation", "condition", "derived-construct"];
const claimCategories = ["core", "constructive", "difficult", "developmental", "interaction"];
const claimConfidences = ["core", "well-supported", "school-specific", "experimental"];
const xmlName = /^[A-Za-z_][A-Za-z0-9_.:-]*$/u;
const decodeEntity = (token, documentName) => {
    switch (token) {
        case "amp": return "&";
        case "lt": return "<";
        case "gt": return ">";
...
const decodeXmlText = (value, documentName) => {
    let output = "";
    let offset = 0;
    while (offset < value.length) {
        ...
        if (end < 0)
            throw new Error(`${documentName}: unescaped ampersand in XML text`);
        output += decodeEntity(value.slice(amp + 1, end), documentName);
        offset = end + 1;
    }
    return output;
};
const parseAttributes = (raw, documentName, elementName) => {
    const attributes = {};
    ...
    while (remaining.length > 0) {
        ...
        if (!xmlName.test(name))
            throw new Error(`${documentName}: invalid XML attribute name ${name}`);
        if (attributes[name] !== undefined)
            throw new Error(`${documentName}: duplicate XML attribute ${name} on <${elementName}>`);
        attributes[name] = decodeXmlText(match[2] ?? match[3] ?? "", documentName);
        remaining = remaining.slice(match[0].length);
    }
    return attributes;
};
const appendText = (node, raw, documentName) => {
    const decoded = decodeXmlText(raw, documentName).replaceAll(/\s+/gu, " ").trim();
    if (decoded.length === 0)
        return;
    node.text = node.text.length === 0 ? decoded : `${node.text} ${decoded}`;
};
const parseXmlDocument = (source, documentName) => {
    ...
    for (const match of source.matchAll(token)) {
        if (match.index !== offset)
            throw new Error(`${documentName}: malformed XML near offset ${offset}`);
        offset = match.index + match[0].length;
        if (match[0].startsWith("<?xml") || match[0].startsWith("<!--"))
            continue;
        const closing = match[1];
        if (closing !== undefined) {
            if (stack.length === 1)
                throw new Error(`${documentName}: unexpected closing tag </${closing}>`);
        ...
        if (selfClosing !== undefined) {
            ...
            if (parent === undefined)
                throw new Error(`${documentName}: XML parser lost its parent node`);
        ...
        const parent = stack.at(-1);
        if (parent === undefined)
            throw new Error(`${documentName}: XML parser lost its text parent`);
        appendText(parent, match[6] ?? "", documentName);
    }
    if (offset !== source.length)
        throw new Error(`${documentName}: malformed XML near offset ${offset}`);
    if (stack.length !== 1)
        throw new Error(`${documentName}: unclosed XML element <${stack.at(-1)?.name ?? "unknown"}>`);
    if (document.text.length > 0)
        throw new Error(`${documentName}: text is not allowed outside the root element`);
...
const assertAttributes = (node, allowed, documentName) => {
    const permitted = new Set(allowed);
    for (const name of Object.keys(node.attributes)) {
        if (!permitted.has(name))
            throw new Error(`${documentName}: unexpected attribute ${name} on <${node.name}>`);
    }
};
const assertChildren = (node, allowed, documentName) => {
    const permitted = new Set(allowed);
    for (const child of node.children) {
        if (!permitted.has(child.name))
            throw new Error(`${documentName}: unexpected <${child.name}> inside <${node.name}>`);
    }
};
const childrenNamed = (node, name) => node.children.filter((child) => child.name === name);
const oneChild = (node, name, documentName) => {
...
const leafText = (node, documentName) => {
    if (node.children.length > 0)
        throw new Error(`${documentName}: <${node.name}> must contain text only`);
    if (Object.keys(node.attributes).length > 0)
        throw new Error(`${documentName}: <${node.name}> does not allow attributes`);
    return node.text;
};
const childText = (node, name, documentName) => leafText(oneChild(node, name, documentName), documentName);
const nullableChildText = (node, name, documentName) => {
    const child = oneChild(node, name, documentName);
    assertAttributes(child, ["null"], documentName);
    if (child.children.length > 0)
        throw new Error(`${documentName}: <${name}> must contain text only`);
    const nullMarker = child.attributes["null"];
    if (nullMarker === undefined)
        return child.text;
    if (nullMarker !== "true")
        throw new Error(`${documentName}: <${name}> null attribute must be true when present`);
    if (child.text.length > 0)
        throw new Error(`${documentName}: null <${name}> cannot also contain text`);
    return null;
};
const listText = (node, containerName, itemName, documentName) => {
    ...
    if (container.text.length > 0)
        throw new Error(`${documentName}: <${containerName}> may contain only <${itemName}> children`);
    return childrenNamed(container, itemName).map((item) => leafText(item, documentName));
};
const requiredAttribute = (node, name, documentName) => {
    const value = node.attributes[name];
    if (value === undefined || value.length === 0)
        throw new Error(`${documentName}: <${node.name}> requires attribute ${name}`);
    return value;
};
const enumAttribute = (node, name, allowed, documentName) => {
...
const verifyFormat = (node, documentName) => {
    const format = requiredAttribute(node, "format", documentName);
    if (format !== corpusXmlFormat)
        throw new Error(`${documentName}: unsupported corpus XML format ${format}`);
};
export const parseCorpusSourceManifestXml = (xml, documentName = "sources.xml") => {
    const root = parseXmlDocument(xml, documentName);
    if (root.name !== "sources")
        throw new Error(`${documentName}: expected <sources> root`);
    assertAttributes(root, ["format"], documentName);
    assertChildren(root, ["source"], documentName);
    verifyFormat(root, documentName);
    if (root.text.length > 0)
        throw new Error(`${documentName}: <sources> cannot contain direct text`);
    return childrenNamed(root, "source").map((source) => {
        ...
        if (source.text.length > 0)
            throw new Error(`${documentName}: <source> cannot contain direct text`);
        return {
...
const parseAtom = (atom, documentName) => {
    ...
    if (atom.text.length > 0)
        throw new Error(`${documentName}: <atom> cannot contain direct text`);
    return {
...
const parseNeutrality = (claim, documentName) => {
    ...
    if (neutrality.text.length > 0)
        throw new Error(`${documentName}: <neutrality> must be empty`);
...
const parseClaim = (claim, documentName) => {
    ...
    if (claim.text.length > 0)
        throw new Error(`${documentName}: <claim> cannot contain direct text`);
    return {
...
export const parseCorpusXmlDocument = (xml, documentName) => {
    const root = parseXmlDocument(xml, documentName);
    if (root.name !== "corpus")
        throw new Error(`${documentName}: expected <corpus> root`);
    assertAttributes(root, ["format", "category"], documentName);
    assertChildren(root, ["atoms", "claims"], documentName);
    verifyFormat(root, documentName);
    if (root.text.length > 0)
        throw new Error(`${documentName}: <corpus> cannot contain direct text`);
    const atoms = oneChild(root, "atoms", documentName);
    assertAttributes(atoms, [], documentName);
    assertChildren(atoms, ["atom"], documentName);
    if (atoms.text.length > 0)
        throw new Error(`${documentName}: <atoms> cannot contain direct text`);
    ...
    if (claims.text.length > 0)
        throw new Error(`${documentName}: <claims> cannot contain direct text`);
    return {
...
export const parseReviewedCorpusXml = (sourceManifestXml, sourceManifestName, documents) => {
    ...
    for (const document of documents) {
        if (seenNames.has(document.name))
            throw new Error(`Duplicate corpus XML document name ${document.name}`);
        ...
        if (seenCategories.has(parsed.category))
            throw new Error(`Duplicate corpus XML category ${parsed.category}`);
