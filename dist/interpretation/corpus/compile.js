...
export const corpusPolicyVersion = "astral-corpus-policy/1.1.0";
export const interpretationCorpusVersion = "astral-interpretation-corpus/0.2.0";
const unique = (values, kind) => {
    const output = new Map();
    for (const value of values) {
        if (output.has(value.id))
...
const parseSourceRef = (ref) => {
    ...
    if (hash < 0)
        return { sourceId: ref, sectionId: null };
    return {
        sourceId: ref.slice(0, hash),
        sectionId: ref.slice(hash + 1) || null,
    };
};
export const validateSourceForSemanticIngestion = (source, passage) => {
...
export const validateCorpusClaim = (claim) => {
    if (claim.neutrality.religious
...
const validateAtom = (atom, sources) => {
    if (atom.reviewStatus !== "approved") {
        throw new Error(`Corpus atom ${atom.id} is not approved for production compilation`);
    }
    if (atom.plainEnglish.trim().length === 0)
        throw new Error(`Corpus atom ${atom.id} has no plain-English semantic label`);
    assertAgnosticText(atom.plainEnglish, `Corpus atom ${atom.id}`);
    if (atom.sourceIds.length === 0)
        throw new Error(`Corpus atom ${atom.id} has no semantic source provenance`);
    for (const sourceId of atom.sourceIds) {
        const source = sources.get(sourceId);
        if (source === undefined)
            throw new Error(`Corpus atom ${atom.id} references unknown source ${sourceId}`);
...
const validateClaimProvenance = (claim, sources) => {
    for (const ref of claim.sourceRefs) {
        const { sourceId, sectionId } = parseSourceRef(ref);
        const source = sources.get(sourceId);
        if (source === undefined)
            throw new Error(`Corpus claim ${claim.id} references unknown source ${sourceId}`);
        if (source.role !== "semantic" || source.reviewStatus !== "approved") {
...
export const compileInterpretationCorpus = (input) => {
    const sources = unique(input.sources, "source");
    const atoms = unique(input.atoms, "atom");
    const claims = unique(input.claims, "claim");
    for (const atom of atoms.values())
        validateAtom(atom, sources);
    for (const claim of claims.values()) {
        validateCorpusClaim(claim);
        validateClaimProvenance(claim, sources);
        const atom = atoms.get(claim.atomId);
        if (atom === undefined)
            throw new Error(`Corpus claim ${claim.id} references unknown atom ${claim.atomId}`);
        if (!atom.claimIds.includes(claim.id)) {
            throw new Error(`Corpus claim ${claim.id} is not declared by atom ${claim.atomId}`);
        }
    }
    for (const atom of atoms.values()) {
        if (atom.claimIds.length === 0)
            throw new Error(`Corpus atom ${atom.id} has no semantic claims`);
        for (const claimId of atom.claimIds) {
            const claim = claims.get(claimId);
            if (claim === undefined)
                throw new Error(`Corpus atom ${atom.id} references missing claim ${claimId}`);
            if (claim.atomId !== atom.id)
                throw new Error(`Corpus atom ${atom.id} owns claim ${claimId} assigned to ${claim.atomId}`);
        }
    }
...
export const validateInterpretationMap = (map) => {
