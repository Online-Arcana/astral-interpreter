...
export const fetchOpenAICosts = async (adminKey, startTime, endTime = null, fetcher = globalThis.fetch.bind(globalThis)) => {
    if (adminKey.trim().length === 0)
        throw new Error("OPENAI_ADMIN_KEY is required for provider cost reconciliation");
    if (!Number.isSafeInteger(startTime) || startTime < 0)
        throw new Error("start_time must be a non-negative Unix timestamp");
    ...
    do {
        ...
        if (!response.ok)
            throw new Error(`OpenAI Costs API failed with HTTP ${response.status}: ${(await response.text()).slice(0, 500)}`);
        const value = await response.json();
        if (!object(value) || !Array.isArray(value["data"]))
            throw new Error("OpenAI Costs API returned an invalid page");
        for (const bucket of value["data"]) {
            if (!object(bucket) || !Array.isArray(bucket["results"]))
                continue;
            ...
            for (const entry of bucket["results"]) {
                if (!object(entry) || !object(entry["amount"]))
                    continue;
...
//# sourceMappingURL=openaiCosts.js.map