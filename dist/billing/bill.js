...
const totals = (events) => {
    const result = zero();
    for (const event of events)
        add(result, event.usage, event.costUsd);
    return result;
};
const copy = (bill) => JSON.parse(JSON.stringify(bill));
export class BillCollector {
    ...
    #status;
    #endedAt;
    constructor(calculationFingerprint, previous = null, now = () => new Date().toISOString(), coverage = previous?.coverage ?? "complete") {
    ...
    finish(status, at = new Date().toISOString()) {
        this.#status = status;
        this.#endedAt = at;
...
export const billingSummary = (values, latest = 10) => {
    ...
    const pricedCompleted = completed.filter((bill) => bill.coverage !== "partial" && bill.total.costUsd !== null);
