...
const billName = (id) => `${id.replaceAll(/[^A-Za-z0-9_-]/gu, "_")}.json`;
const parseBill = (text) => {
    try {
...
export class BillStore {
    ...
    constructor(dir) {
        if (dir.trim().length === 0)
            throw new Error("Billing directory is required");
        this.#dir = dir;
    }
    live(bill) {
    ...
    async list() {
        ...
        for (const bill of saved)
            if (bill !== null)
                merged.set(bill.id, bill);
        for (const bill of this.#live.values())
            merged.set(bill.id, structuredClone(bill));
