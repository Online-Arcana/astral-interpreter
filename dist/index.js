...
var parseSexualInterpretation = (input) => {
...
var parseCareerInterpretation = (input) => {
...
var parseMoneyInterpretation = (input) => {
...
var parseSystemSynthesis = (input) => {
...
var parseCompatibilityOverview = (input) => {
...
var parseSignCompatibility = (input, expectedSign2) => {
...
var refsValid = (root2, refs3, allowed) => refs3.every((ref2) => {
  ...
  try {
    ...
    if (value && typeof value === "object" && "status" in value) {
      const status3 = value.status;
      return status3 !== "unavailable" && status3 !== "unsupported";
    }
    return true;
  } catch {
    return false;
...
var compileInterpretationCorpus = (input) => {
  const sources2 = unique(input.sources, "source");
...
var parseAttributes = (raw, documentName, elementName) => {
  const attributes = {};
...
var appendText = (node, raw, documentName) => {
...
var pointFromSection = (unit2) => {
  const raw = unit2.section.startsWith("points.") ? unit2.section.slice("points.".length) : unit2.section.startsWith("bigThree.") ? unit2.section.slice("bigThree.".length) : null;
  return pointId(raw) ? raw : null;
};
var signFromCompatibilityId = (unit2) => {
  if (unit2.section !== "compatibility.sign") return null;
  const raw = unit2.id.split(".").at(-1);
...
var OpenAIError = class extends Error {
  status;
  body;
  constructor(status3, body) {
    super(`OpenAI request failed with status ${status3}`);
...
var OutputError = class extends Error {
  rawText;
  attempts;
  constructor(message, rawText2, attempts) {
    super(message);
...
var OpenAISchema = class {
  ...
  async send(input, opts) {
    return this.push(() => this.call(this.current, input, opts));
  }
  async run(schema2, input, opts, name = "DynamicSchema") {
    return this.push(async () => {
  ...
  async call(shape2, input, opts) {
    ...
    throw new OutputError(`Could not parse the expected output after ${attempts} attempt${attempts === 1 ? "" : "s"}: ${last}`, raw, attempts);
  }
};
...
var OpenAITransportError = class extends Error {
  ...
  responseStatus;
  timedOut;
  constructor(message, responseId = null, responseStatus2 = null, cause, timedOut = false) {
...
var statusOf = (value) => {
  const status3 = value["status"];
  if (status3 === "queued" || status3 === "in_progress" || status3 === "completed" || status3 === "failed" || status3 === "cancelled" || status3 === "incomplete") {
    return status3;
  }
  return null;
};
...
var requestUrl = (input) => input instanceof Request ? input.url : String(input);
var requestMethod = (input, init) => String(
  init?.method ?? (input instanceof Request ? input.method : "GET")
).toUpperCase();
var responseEndpoint = (input) => {
...
var trimBody = async (response) => (await response.text()).replaceAll(/\s+/gu, " ").trim().slice(0, 500);
var responseFailure = (id, status3, value) => new OpenAITransportError(
  `OpenAI response ${id} ${status3}: ${errorMessage(value, "no further detail")}`,
  id,
  status3
);
var httpFailure = async (response, id) => {
  const detail2 = await trimBody(response);
  return new OpenAITransportError(
    `OpenAI polling failed with HTTP ${response.status}${detail2.length === 0 ? "" : `: ${detail2}`}`,
    id,
    null
  );
};
var createOpenAITransport = (options = {}) => {
  ...
  const request = async (input, init, timeoutMs, message, responseId, responseStatus2) => {
  ...
  const poll = async (responseUrl, id, headers, signal) => {
    ...
    for (; ; ) {
      ...
      const status3 = statusOf(value);
      if (status3 === "completed") {
        return json(response, value);
      }
      if (status3 !== null && waiting.has(status3)) {
        continue;
      }
      if (status3 !== null && terminal.has(status3)) {
  ...
  const send = async (input, init, body, signal, responseAttempt) => {
    ...
    const status3 = statusOf(value);
    if (status3 === null || status3 === "completed") {
      return json(response, value);
    }
    ...
    if (!waiting.has(status3)) {
      throw responseFailure(id, status3, value);
    }
    const responseUrl = requestUrl(input).replace(/\/+$/u, "");
    try {
      return await poll(
        responseUrl,
        id,
  ...
  return async (input, init) => {
    if (!background || requestMethod(input, init) !== "POST" || !responseEndpoint(input) || typeof init?.body !== "string") {
      return request(
        input,
        init ?? {},
        pollTimeoutMs,
...
var salvagePartialJsonObject = (raw) => {
...
var responseError = (cause) => {
  ...
  const status3 = cause["responseStatus"];
  return typeof id === "string" && id.length > 0 ? { id, incomplete: status3 === "incomplete" } : null;
};
...
var estimateContextTokens = (input) => {
      ...
      try {
        ...
        if (response?.incomplete === true) {
          try {
            ...
            if (partial.length > 0 && record5(cause)) cause["rawText"] = partial;
          } catch {
          }
        }
...
var parseSection = (value) => {
  ...
  const status3 = value["status"];
  if (status3 !== "written" && status3 !== "unavailable" && status3 !== "not_applicable") throw new TypeError("Invalid section status");
...
var cleanConcept = (raw) => {
  const value = raw.replaceAll("_", " ").replaceAll(/\s+/gu, " ").trim();
...
var fallbackText = (family2, key, unit2, schema2) => {
  ...
  const raw = familyFields[key] ?? commonFields[key] ?? (key === "title" ? "{topic}" : "You may approach {topicLower} with growing awareness and flexibility.");
  const topic = topicFor(unit2);
  const sign = constString(schema2) ?? human(unit2.id.split(".").at(-1) ?? "this sign");
  return raw.replaceAll("{topicLower}", topic.toLocaleLowerCase()).replaceAll("{topic}", topic).replaceAll("{sign}", sign.charAt(0).toLocaleUpperCase() + sign.slice(1));
};
...
var snapshotInput = (fileId, snapshot, input) => {
...
var paidAttempts = 2;
var entryModelFor = (config, kind) => kind === "big" ? config.openai.bigModel : config.openai.smallModel;
var escalationModelFor = (config, kind) => kind === "big" ? config.openai.bigEscalationModel : config.openai.smallEscalationModel;
var modelFor = (config, unit2, attempt) => attempt <= 1 ? entryModelFor(config, unit2.kind) : escalationModelFor(config, unit2.kind);
var effortFor = (config, unit2, attempt) => attempt > 1 && unit2.kind === "small" ? "low" : unit2.effort ?? config.openai.reasoning;
var tokensFor = (config, unit2) => Math.min(unit2.tokens ?? config.openai.maxOutputTokens, config.openai.maxOutputTokens);
...
var objectCandidate = (value) => typeof value === "object" && value !== null && !Array.isArray(value) ? value : null;
var candidateFromCause = (cause) => {
  const raw = rawText(cause).trim();
...
var callInput = (unit2, context, snapshot, remoteFileId) => {
  const input = unit2.input(context);
  return snapshot === null ? input : snapshotInput(remoteFileId, snapshot, input);
};
var safeAudit = (unit2, value, context) => {
  try {
    return unit2.audit(value, context);
  } catch (cause) {
    return {
      valid: false,
      value,
...
var resolveWorldviewReview = async (options, audit) => {
...
var reconstructionResult = async (options, candidates, context, attempt, model, errors) => {
...
var executeUnit = async (options) => {
  ...
  if ((options.resume?.attempt ?? 1) > paidAttempts) {
    ...
    return reconstructionResult(options, candidates, context2, paidAttempts, lastModel, correction);
  }
  for (let attempt = firstAttempt; attempt <= paidAttempts; attempt += 1) {
    ...
    try {
      output = await options.limiter.run(() => options.client.run(
        options.unit.shape,
        callInput(options.unit, context2, options.snapshot, options.remoteFileId),
        {
          body: {
            model,
            store: false,
      ...
      return reconstructionResult(options, candidates, context2, attempt, model, correction);
    }
    const audited = await resolveWorldviewReview(options, safeAudit(options.unit, output, context2));
...
var ProgressTracker = class {
  ...
  start(id, status3, nowMs, attempt = 1, modelName = null) {
    const unit2 = this.#units.find((candidate) => candidate.id === id);
...
var weightFor = (unit2) => {
  const tokens = unit2.tokens ?? (unit2.kind === "big" ? 3200 : 1800);
...
var progressHooks = (tracker, now, emit, accepted = []) => {
  ...
  return {
    onStart: (unit2, attempt, model) => {
      tracker.start(unit2.id, "interpreting", now(), attempt, model);
...
var serialiseInterpretationPrompt = (input) => ({
  ...
  ...input.correction === void 0 || input.correction === null ? {} : {
    correction: {
...
var acceptedNarratives = (earlier) => Object.entries(earlier).flatMap(([id, raw]) => {
...
var runPlan = async (calculation, config, createClient, hooks, recovery, semanticProvider) => {
  ...
  const raw = calls.length === 0 ? localRun(prepared.synthetic, "All interpretation units used deterministic fallback") : await runInterpretation2(root(calculation), calls, config, createClient, hooks, recovery);
  ...
  const units = {};
  for (const unit2 of calculation.interpretationPlan.units) {
    const value = raw.units[unit2.id] ?? prepared.synthetic[unit2.id] ?? sourceAwareFallback(
      calculation,
      unit2,
...
var diagnosticHooks = (hooks, now) => {
  ...
  return {
    ...hooks,
    onStart: (unit2, attempt, model) => {
      hooks.onStart?.(unit2, attempt, model);
      void emit({
        ...base("start", now, wave),
        unitId: unit2.id,
        attempt,
    ...
    onReject: async (unit2, attempt, model, output, audit) => {
      await hooks.onReject?.(unit2, attempt, model, output, audit);
      await emit({
    ...
    onWave: async (value) => {
      ...
      await emit({
        ...
        failureKind: value.lanes.find(({ failureKind: failureKind2 }) => failureKind2 !== null)?.failureKind ?? null
      });
    }
  };
};
...
var modelRoot = (model, catalogue) => {
  const matches = Object.keys(catalogue.models).filter((key) => model === key || model.startsWith(`${key}-`)).sort((left, right) => right.length - left.length);
  return matches[0] ?? null;
};
var rateFor = (model, catalogue = openAiPriceCatalogue) => {
  const root2 = modelRoot(model, catalogue);
  return root2 === null ? null : catalogue.models[root2] ?? null;
};
var priceUsage = (model, usage, catalogue = openAiPriceCatalogue) => {
...
var BillCollector = class {
  ...
  finish(status3, at = (/* @__PURE__ */ new Date()).toISOString()) {
    this.#status = status3;
    this.#endedAt = at;
...
var billingSummary = (values, latest = 10) => {
  ...
  const completed = bills.filter(({ status: status3 }) => status3 === "completed");
  ...
  return {
    ...
    failedBills: bills.filter(({ status: status3 }) => status3 === "failed").length,
...
var fetchOpenAICosts = async (adminKey, startTime, endTime = null, fetcher = globalThis.fetch.bind(globalThis)) => {
  ...
  do {
    ...
    const value = await response.json();
    if (!object2(value) || !Array.isArray(value["data"])) throw new Error("OpenAI Costs API returned an invalid page");
    for (const bucket2 of value["data"]) {
      if (!object2(bucket2) || !Array.isArray(bucket2["results"])) continue;
      ...
      for (const entry2 of bucket2["results"]) {
        if (!object2(entry2) || !object2(entry2["amount"])) continue;
