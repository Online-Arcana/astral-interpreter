...
import { activeCopy, conversation, count, executeUnit, localConversationId, paidAttempts, reconstructionResult } from "./execute.js";
import { emptyRecovery, emergencyRun, laneCheckpoint, recoveredPlans, restore, restoreStaged, without } from "./recovery.js";
const runCore = async (calculation, calls, config, createClient, hooks, recovery) => {
    if (calls.length === 0)
        throw new Error("Interpretation requires at least one unit");
    if (new Set(calls.map(({ id }) => id)).size !== calls.length)
        throw new Error("Interpretation unit IDs must be unique");
    ...
    if (!foundationComplete) {
        ...
        for (const unit of foundation) {
            ...
            if (contextTokens > 0 && contextTokens + estimate > (config.chart.laneContextTokens ?? 60_000))
                break;
    ...
    while (Object.keys(completed).length < calls.length) {
        ...
        const plans = resumingWave
            ? recoveredPlans(calls, currentWave)
            : wavePlan(calls, completed, config.chart.laneCount ?? 4, config.chart.laneUnits ?? 10);
        if (plans.length === 0)
            throw new Error("Interpretation planner could not produce a dependency-safe wave");
        ...
        if (remoteFileId === null && uploader.uploadFile !== undefined) {
            ...
            const uploaded = await limiter.run(() => upload(`astral-snapshot-${snapshot.revision}.json`, snapshotText(snapshot)));
        ...
        const laneRuns = plans.map(async (plan, index) => {
            ...
            if (lane === undefined)
                throw new Error(`Missing checkpoint for ${plan.id}`);
            ...
            for (const unit of plan.units) {
                ...
                if (existing !== undefined) {
                    ...
                    if (!lane.completed.includes(unit.id))
        ...
        if (waveIssues.length > 0) {
            ...
            for (const id of affected) {
                ...
                if (unit === undefined || prior === undefined)
                    continue;
...
//# sourceMappingURL=run-v2.js.map