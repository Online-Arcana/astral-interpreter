...
const substantiveCalls = (calculation, semanticProvider) => {
    const calls = [];
    const synthetic = {};
    for (const unit of calculation.interpretationPlan.units) {
        ...
        calls.push({
            ...
            audit: (value, context) => auditStructured(value, context.calculation, allowed, { ...profile, priorFields: acceptedNarratives(context.earlier) }),
        });
    }
...
const generatedNameCall = (calculation) => {
    ...
    return {
        ...
        effort: "none",
        tokens: 128,
        shape: strictShape("generated_chart_name", object({ value: text() }), (value) => {
...
export const interpretationCalls = (calculation, semanticProvider = null) => {
