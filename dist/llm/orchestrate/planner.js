const pending = (calls, accepted) => calls.filter(({ id }) => accepted[id] === undefined);
export const foundationPlan = (calls, accepted, maximum = 10) => {
    if (!Number.isSafeInteger(maximum) || maximum < 1)
        throw new Error("Foundation maximum must be a positive integer");
    return pending(calls, accepted).slice(0, maximum);
};
const dependenciesMet = (call, accepted, lane) => {
    const local = new Set(lane.map(({ id }) => id));
    return (call.dependsOn ?? []).every((id) => accepted.has(id) || local.has(id));
};
const weight = (call) => call.tokens ?? (call.kind === "big" ? 3_200 : 1_800);
export const wavePlan = (calls, acceptedUnits, maximumLanes = 4, maximumPerLane = 10) => {
    ...
    while (changed) {
        ...
        for (const call of remaining) {
            if (lanes.some((lane) => lane.units.some(({ id }) => id === call.id)))
                continue;
            ...
            changed = true;
            if (lanes.reduce((count, item) => count + item.units.length, 0) >= maximumLanes * maximumPerLane)
                break;
        }
    }
