export const interpretationSnapshotSchema = "astral-interpretation-snapshot/1.0.0";
const record = (value) => typeof value === "object" && value !== null && !Array.isArray(value);
const fingerprint = (calculation) => {
    if (!record(calculation))
        return null;
    const root = record(calculation["astral-calculation"])
        ? calculation["astral-calculation"]
        : calculation;
...
export const buildSnapshot = async (calculation, accepted, order, revision) => {
...
export const snapshotText = (snapshot) => JSON.stringify(snapshot);
/**
 ...
 */
export const snapshotTokenEstimate = (snapshot) => Math.max(1, Math.ceil(snapshotText(snapshot).length / 3));
export const snapshotInput = (fileId, snapshot, input) => {
    if (fileId === null)
