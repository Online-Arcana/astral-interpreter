...
import { fallbackCatalogue } from "./catalogue.js";
import { semanticFallbackText } from "./semantic.js";
const record = (value) => typeof value === "object" && value !== null && !Array.isArray(value);
const human = (value) => value
...
const schemaProperties = (schema) => {
    ...
    if (!record(value))
        return {};
    return Object.fromEntries(Object.entries(value).filter((entry) => record(entry[1])));
};
const familyFor = (unit) => {
    if (unit.id === "generated-name")
        return "generated-name";
    const fields = new Set(Object.keys(schemaProperties(unit.shape.schema)));
    if (fields.has("closingPortrait"))
        return "final-synthesis";
    if (fields.has("bestExpression"))
        return "compatibility-sign";
    if (fields.size === 2 && fields.has("overview") && fields.has("sourceRefs"))
        return "compatibility-overview";
    if (fields.has("centralThemes") && fields.has("narrative"))
        return "synthesis";
    if (fields.has("desireStyle"))
        return "sexuality";
    if (fields.has("affectionStyle"))
        return "romance";
    if (fields.has("vocationalThemes"))
        return "career";
    if (fields.has("earningStyle"))
        return "money";
    return "section";
};
const constString = (schema) => typeof schema["const"] === "string" ? schema["const"] : null;
const enumString = (schema) => {
...
const valueType = (schema) => typeof schema["type"] === "string" ? schema["type"] : null;
const stripProcessNarration = (value) => value
...
const lastCompleteSentence = (value) => {
    const matches = [...value.matchAll(/[^.!?]+[.!?](?=\s|$)/gu)];
    return matches.at(-1)?.[0]?.trim() ?? "";
};
const directAddress = (value) => {
    if (/\b(?:you|your|yourself)\b/iu.test(value))
        return value;
...
const cleanText = (raw, key) => {
    ...
    if (!worldview.safe || worldview.requiresReview)
        return null;
    if (incompleteEnding.test(value))
        value = lastCompleteSentence(value);
    if (value.length === 0)
        return null;
    if (key !== "title" && key !== "value" && key !== "sign" && !value.startsWith("#/")) {
        value = directAddress(value);
    }
    if (key !== "title" && key !== "value" && key !== "sign" && !/[.!?]$/u.test(value))
        value += ".";
    return value;
};
const candidateValues = (candidates, key) => candidates
    .slice()
    .reverse()
    .flatMap((candidate) => record(candidate) && key in candidate ? [candidate[key]] : []);
const fallbackText = (family, key, unit, schema) => {
...
const allowedRefs = (unit) => [...unit.allowedSourceRefs];
const validRef = (value, unit) => typeof value === "string" && unit.allowedSourceRefs.has(value);
const minItems = (schema) => typeof schema["minItems"] === "number" && Number.isSafeInteger(schema["minItems"])
    ? Math.max(0, schema["minItems"])
    : 0;
const maxItems = (schema) => typeof schema["maxItems"] === "number" && Number.isSafeInteger(schema["maxItems"])
    ? Math.max(0, schema["maxItems"])
    : null;
const scalarFallback = (state, key, schema) => {
...
const buildString = (state, key, schema, values) => {
...
const buildArray = (state, key, schema, values) => {
    const itemSchema = record(schema["items"]) ? schema["items"] : { type: "string" };
    if (key === "sourceRefs") {
        const refs = values
            .flatMap((value) => Array.isArray(value) ? value : [])
            .filter((value) => validRef(value, state.unit));
        const unique = [...new Set(refs)];
        if (unique.length > 0 && !state.forceFields.has(key))
            return unique;
        ...
        for (const raw of values) {
            ...
            for (const item of raw) {
                if (valueType(itemSchema) === "string" || nullable(itemSchema)) {
                    ...
                    if (cleaned !== null && !output.includes(cleaned))
                        output.push(cleaned);
                }
                else if (record(item)) {
                    output.push(item);
...
const buildValue = (state, key, schema, values) => {
    ...
    if (type === "array")
        return buildArray(state, key, schema, values);
    if (type === "string")
        return buildString(state, key, schema, values);
    if (nullable(schema)) {
        const cleaned = state.forceFields.has(key)
            ? null
            : values.map((value) => cleanText(value, key)).find((value) => value !== null);
        return cleaned ?? scalarFallback(state, key, schema);
    }
    state.warnings.push(`Unsupported reconstruction schema at ${key}`);
    return scalarFallback(state, key, schema);
};
const absoluteValue = (unit, family, key, schema) => {
    const constant = constString(schema);
    if (constant !== null)
        return constant;
    const selected = enumString(schema);
    ...
    if (Array.isArray(variants)) {
        ...
        if (record(concrete))
            return absoluteValue(unit, family, key, concrete);
        return null;
    }
    const type = valueType(schema);
    if (type === "object") {
    ...
    if (type === "array") {
        if (key === "sourceRefs")
            return allowedRefs(unit).slice(0, Math.max(1, minItems(schema)));
        const item = record(schema["items"]) ? schema["items"] : { type: "string" };
...
export const fieldsFromAuditErrors = (unit, errors) => {
...
export const reconstructUnit = ({ unit, candidates, forceFields = new Set(), }) => {
    try {
        const state = {
        ...
        const family = unit.id === "generated-name" ? "generated-name" : "section";
