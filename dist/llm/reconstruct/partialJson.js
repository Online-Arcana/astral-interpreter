...
const appendable = (segments, segment) => parsedObject(`{${[...segments, segment].join(",")}}`) !== null;
/**
 ...
 */
export const salvagePartialJsonObject = (raw) => {
    ...
    for (let index = 0; index < source.length; index += 1) {
        ...
        if (character === "}" || character === "]") {
            if (character === "}" && depth === 1 && segmentStart >= 0) {
                ...
                if (segment.length > 0 && appendable(segments, segment))
...
//# sourceMappingURL=partialJson.js.map