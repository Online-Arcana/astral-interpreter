...
const cleanConcept = (raw) => {
    const value = raw
        ...
        .replaceAll(/\s+/gu, " ")
        .trim();
    if (value.length < 2 || value.length > 80 || machineLike.test(value))
        return null;
    const worldview = auditWorldviewText(value);
    return worldview.safe && !worldview.requiresReview ? value : null;
};
const bucket = (map, names) => names.flatMap((name) => map.semantics[name]);
const concepts = (map, names) => {
...
const phrase = (values) => {
    if (values.length === 0)
        return "the patterns described here";
    if (values.length === 1)
        return values[0];
    if (values.length === 2)
...
const safeMapOwnedFallback = (map, key) => {
    if (key === "title")
        return map.subject.title;
    const domain = cleanConcept(map.subject.plainEnglishDomain)
 ...
 */
export const semanticFallbackText = (map, key) => {
    if (key === "title")
        return map.subject.title;
    const buckets = constructive.has(key)
    ...
    if (fallbackAudit.safe && !fallbackAudit.requiresReview)
        return fallback;
