...
const countFrames = (value, patterns) => patterns.reduce((count, pattern) => count + (pattern.test(value) ? 1 : 0), 0);
export const semanticRole = (id) => {
...
const scoreRole = (value, role) => countTerms(value, roleTerms(role)) + countFrames(value, roleFrames(role));
export const semanticIssues = (value, profile) => {
    const normal = normaliseText(value);
    if (normal.split(" ").filter(Boolean).length < 8)
        return [];
    const issues = [];
